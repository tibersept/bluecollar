...
public class TimeRange {
	 ...
	 */
	public List<Range<Long>> getTimeRanges( String aUser, String aProject, Date aDay ) {
	 ...
	 */
	public List<Range<Long>> getTimeRanges( String aUser, String aProject, Date aBegin, Date anEnd ) {
	...
	/**
	 * Retrieves all time ranges of the given project assigned to user.
	 * @param aUser the user
	 * @param aProject the project
	 * @return the list of time range entities
	 */
	public List<Entity> getRangeEntities( String aUser, String aProject ) {
		Entity pro = project.getProject(aUser, aProject);
		if( pro!=null ) {
			Key proKey = pro.getKey();
			Query q = new Query(TIME_RANGE, proKey).setAncestor(proKey);
	...
	/**
	 * Retrieves all time ranges of the given project which have started on the given day. 
	 * @param aUser the user
	 * @param aProject the project
	 * @param aDay the day
	 * @return the list of time range entities for the project of the user on that day
	 */
	public List<Entity> getRangeEntities( String aUser, String aProject, Date aDay ) {
		Entity pro = project.getProject(aUser, aProject);
		if( pro!=null ) {
			Key proKey = pro.getKey();
			Calendar cal = getCal();
	 ...
	 */
	public List<Entity> getRangeEntities( String aUser, String aProject, Date aBegin, Date anEnd ) {
		Entity pro = project.getProject(aUser, aProject);
		if( pro!=null ) {
			Key proKey = pro.getKey();
			Calendar cal = getCal();
	 ...
	 */
	public void openTimeRange( String aUser, String aProject, Date aTimestamp ) {
		...
		if (key!=null) {
			if( !hasOpenRange(key) ) {
				...
				long begin = cal.getTimeInMillis();
				timeRange.setProperty(PROPERTY_START, begin);
	 ...
	 */
	public void closeTimeRange( String aUser, String aProject, Date aTimestamp ) {
		Key key = project.getKey(aUser, aProject);
		if (key!=null) {
			if( hasOpenRange(key) ) {
	...
	private List<Range<Long>> getRangeListFromEntities(List<Entity> entityList) {
		...
		for( Entity entityRange : entityList ) {
			Long begin = (Long) entityRange.getProperty(PROPERTY_START);
	...
	private Entity createNewTimeRange(Key aProjectKey) {
		...
		project.setProperty(PROPERTY_START, null);
		project.setProperty(PROPERTY_END,null);
		service.put(project);
		return project;
	}
	...
	private List<Entity> getTimeRangesInRange(Key aProKey, long aBegin, long anEnd) {
		Filter beginFilter = new FilterPredicate(PROPERTY_START, FilterOperator.GREATER_THAN, aBegin);
		Filter endFilter = new FilterPredicate(PROPERTY_START, FilterOperator.LESS_THAN, anEnd);
